...
class VAEControlNetConditioningEmbedding(nn.Module):
    def __init__(
        self,
        conv_unet: nn.Conv2d,
        autoencoder: Optional[AutoencoderKL] = None,
    ):
        super().__init__()
        self.autoencoder = autoencoder
        self.conv_vae_out = zero_module(conv_unet)

    def forward(self, conditioning):
        embedding = self.autoencoder.encode(conditioning).latent_dist.sample()
        embedding = embedding * self.autoencoder.config.scaling_factor
        embedding = self.conv_vae_out(embedding)
        return embedding
...
class CachedControlNetModel(ControlNetModel):
    def forward(
        self,
        ...
        # if shape of controlnet_cond is same as sample, then we can add it directly
    ...
    def preprocess_image(self, image):
        return self.controlnet_cond_embedding(image)

class FusedControlLoRAModel(ControlNetModel):
    def __init__(self,
        in_channels: int = 4,
        ...
        if uses_vae:
            self.controlnet_cond_embedding = VAEControlNetConditioningEmbedding(
                autoencoder=None, conv_unet=self.conv_in
            )  # type: ignore

    def set_autoencoder(self, autoencoder: AutoencoderKL):
        if type(self.controlnet_cond_embedding) is VAEControlNetConditioningEmbedding:
            self.controlnet_cond_embedding.autoencoder = autoencoder
        else:
            self.controlnet_cond_embedding = VAEControlNetConditioningEmbedding(
                autoencoder=autoencoder, conv_unet=self.conv_in
            )  # type: ignore

class ControlLoRAModel(CachedControlNetModel):
    """
    A ControlLoRA model.
    ...
    def __init__(
        ...
        for name, attn_processor in list(modules.items()):
            ...
            if isinstance(attn_processor, nn.Linear) and not isinstance(
            ...
            elif lora_linear_rank > 0 and isinstance(
                attn_processor, LoRACompatibleLinear
            ):
    ...
    def remove_autoencoder(self, state_dict: Mapping[str, Any]):
        new_state_dict = OrderedDict()
        for k, v in state_dict.items():
            if "autoencoder" not in k:
                new_state_dict[k] = v
        return new_state_dict
    ...
    def from_unet(
        ...
        controllora: ControlLoRAModel = cls(
            ...
            uses_vae=autoencoder is not None,
        )

        if autoencoder is not None:
            controllora.set_autoencoder(autoencoder)
        controllora.tie_weights(unet)

        return controllora

    
    def fuse_lora(self, lora_scale: float = 1.0, safe_fusing: bool = False):
        for name, attn_processor in list(self.named_modules()):
            if isinstance(
                attn_processor, LoRACompatibleConv
            ):
                attn_processor._fuse_lora(lora_scale=lora_scale, safe_fusing=safe_fusing)
            if isinstance(
                attn_processor, LoRACompatibleLinear
            ):
                attn_processor._fuse_lora(lora_scale=lora_scale, safe_fusing=safe_fusing)
            
    def fuse(self) -> FusedControlLoRAModel:
        controlnet = FusedControlLoRAModel(
            ...
            mid_block_scale_factor=self.config.mid_block_scale_factor,
            ...
            addition_embed_type=self.config.addition_embed_type,
            ...
            controlnet_conditioning_channel_order=self.config.controlnet_conditioning_channel_order,
            conditioning_embedding_out_channels=self.config.conditioning_embedding_out_channels,
            ...
            uses_vae=self.config.uses_vae,
        )
        self.fuse_lora()
        controlnet.set_autoencoder(self.controlnet_cond_embedding.autoencoder)
        controlnet.load_state_dict(super().state_dict())
        return controlnet
