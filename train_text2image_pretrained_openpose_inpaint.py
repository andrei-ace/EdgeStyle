...
from model.utils import CollateFn, CONDITIONING_IMAGES_TRANSFORMS, BG_COLOR
...
def log_validation(
    ...
    for (
        ...
        guidance_scales = np.linspace(3.0, 7.5, args.num_validation_images)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                while len(weights) > 0:
                    ...
                    assert_dict_equals(model.state_dict(), load_model.state_dict())
                    assert_dict_equals(
                        model.nets[2].state_dict(), load_model.nets[2].state_dict()
                    )
                    assert_dict_equals(
                        model.nets[4].state_dict(), load_model.nets[4].state_dict()
                    )

                    del load_model
                    i -= 1

        def assert_dict_equals(state_dict1, state_dict2):
            for state1, state2 in zip(state_dict1, state_dict2):
                state1_value = state_dict1[state1].to(accelerator.device)
                state2_value = state_dict2[state2].to(accelerator.device)
                if (state1_value != state2_value).any():
                    print(f"State is not equal for {state1} and {state2}")
                    print(f"Actual Model Weights: {state1_value[0:4]}")
                    print(f"Loaded Model Weights: {state2_value[0:4]}")
        ...
        def load_model_hook(models, input_dir):
            ...
            while len(models) > 0:
                ...
                state_dict1 = model.nets[2].state_dict()
                state_dict2 = model.nets[4].state_dict()
                for state1, state2 in zip(state_dict1, state_dict2):
                    state1_value = state_dict1[state1]
                    state2_value = state_dict2[state2]
                    if (state1_value != state2_value).any():
                        raise Exception("States are not equal for controlloras 2 and 4")
