...
def log_validation(
    ...
    agnostic_list = validation_batch["agnostic"]
    ...
    i = 0
    for original, agnostic, original_openpose, clothes, clothes_openpose, prompt in zip(
        original_list,
        agnostic_list,
            ...
            while len(models) > 0:
                ...
                for j, net in enumerate(model.nets):
                    load_model = ControlLoRAModel.from_pretrained(
                        input_dir, subfolder=f"controlnet-{j}", uses_vae=net.uses_vae
                    )
                    if net.uses_vae:
